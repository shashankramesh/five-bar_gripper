...
ofstream log_file("../verify_codes/log/sense_gripp.csv");
...
void linear_motion_gripp(Vector<double, 2>& pI_r, Vector<double, 2>& pF_r, double pm_r[2], double& path_time_r, double& n_r, 
          ...
          std::vector<MoteusCommand>& cmds, std::vector<MoteusResponse>& resp)
{
  ...
  linear_motion_gripp(P_open_r, P_close_r, pm_r, path_time_r, n_r, P_open_l, P_close_l, pm_l, path_time_l, n_l, true, right_finger_kinematics, left_finger_kinematics, pi3_interface, cmds, resp);
...
int main(void)
{
  ...
  pF_r_gripp << -0.005, 0.03;
  pI_l_gripp << -0.02, 0.04;
  pF_l_gripp << 0.005, 0.03;
  ...
  std::cout << "gripp" << std::endl;

  F_r << -0, 0;
  F_l << 0, 0;
  hold_time = 33;

  gripp(pI_r_gripp, pI_l_gripp, pF_r_gripp, pF_l_gripp, F_r, F_l, hold_time, right_finger_kinematics, left_finger_kinematics, pi3_interface, cmds, resp);
