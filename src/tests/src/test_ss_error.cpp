...
int main(int argc, char** argv)
{
  ...
  points.open("workspace_points_5bar/workspace_points.csv", ios::in);

  ofstream log("max_tracking_freq_5bar/ss_error_23_02_22_1kg_v1.csv");
  ...
  {  
    ...
    while(time <= total_time)
    {
      ...
      if(time >= total_time - time_notify)
      {
        if(once)
        {
          printf("Program ending in: %d s\n", time_notify);
          once = false;
        }
      }
