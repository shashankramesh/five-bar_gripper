...
int main(int argc, char** argv)
{
  ...
  double CFD_min = 20*M_PI/180;
  double CFD_max = 150*M_PI/180;
  double CFDangle = 0;
...
// time <= total_time
  while(true)
  {
    ...
    CFDangle = five_bar_kinematics.CFDAngle(conf_feed);
    
    if(!five_bar_kinematics.checkInputSingularSafe(conf_feed, CFD_min, CFD_max))
    {
      std::cout << "Encountering Input Singularity!" << std::endl;
    }
    ...
    std::cout << "Feedback configuration: " << conf_feed << std::endl;
    std::cout << "CFD Angle: " << CFDangle << std::endl;
